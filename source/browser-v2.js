...
host.BrowserHost = class {
    ...
    async start() {
      ...
      xtypes.set('Uint64', base.Uint64)
      xtypes.set('Complex64', base.Complex64)
      xtypes.set('Complex128', base.Complex128)

      const viewTensorHandlers = {
        get(target, key, receiver) {
          ...
          if (res.xtype && res.xtype === 'Uint8Array') {
            return new Uint8Array(res.value.match(/.{1,2}/g).map(byte => parseInt(byte, 16)));
          }
      ...
      const handlers = {
        get(target, key, receiver) {
          ...
          if (res === undefined && typeof key === 'string' && !key.startsWith('_')) {
            res = Reflect.get(target, `_${key}`, receiver);
          }
          ...
          if (res.xtype && xtypes.has(res.xtype)) {
            const ctor = xtypes.get(res.xtype);
            const num = new ctor();
            Object.assign(num, res.value);
            return num;
          }
          ...
          // Tensor
          if (key === 'initializer' && module.Tensor) {
            const descriptors = {
              ...Object.getOwnPropertyDescriptors(module.Tensor.prototype),
            }
            ...
            if (res._type) {
              res._type = new Proxy(res._type, handlers)
            }

            return new Proxy(res, viewTensorHandlers)
          }
